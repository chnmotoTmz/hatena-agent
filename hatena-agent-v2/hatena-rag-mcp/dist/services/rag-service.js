...
export class HatenaRAGService {
    ...
    async findRelatedArticles(query, similarityThreshold = 0.7, maxResults = 5) {
    ...
    async findSemanticallySimilarArticles(query, similarityThreshold, maxResults) {
        if (!this.openai) {
    ...
    async findKeywordBasedRelatedArticles(query, maxResults) {
    ...
    calculateKeywordScore(article, keywords) {
    ...
    getMatchingKeywords(article, keywords) {
        const text = this.getArticleText(article).toLowerCase();
        return keywords.filter(keyword => text.includes(keyword.toLowerCase()));
    }
    async generateSummary(articleIds, summaryType) {
        const articles = [];
    ...
    async generateAISummary(articles, summaryType) {
        if (!this.openai) {
            throw new Error('OpenAI not initialized');
        }
        const articlesText = articles.map(article => `タイトル: ${article.title}\n日付: ${article.date}\nURL: ${article.url}\n内容: ${article.content || article.summary}`).join('\n\n---\n\n');
        let prompt = '';
        switch (summaryType) {
            case 'brief':
    ...
    generateSimpleSummary(articles, summaryType) {
