...
class HatenaRAGServer {
    ...
    async handleExtractArticles(args) {
    ...
    async handleSearchArticles(args) {
        ...
        return {
            content: [{
                    ...
                    text: `Found ${results.length} articles matching the search criteria:\n\n` +
                        results.map(article => `• ${article.title}\n  URL: ${article.url}\n  Date: ${article.date}\n`).join('\n')
                }]
        };
    }
    async handleGetArticleDetails(args) {
    ...
    async handleRetrieveRelatedArticles(args) {
        const { query, similarity_threshold = 0.7, max_results = 5 } = args;
        const relatedArticles = await this.ragService.findRelatedArticles(query, similarity_threshold, max_results);
        return {
            content: [{
                    type: 'text',
                    text: `Found ${relatedArticles.length} related articles:\n\n` +
                        relatedArticles.map(({ article, score }) => `• ${article.title} (similarity: ${score.toFixed(3)})\n  ${article.url}\n`).join('\n')
                }]
        };
    }
    async handleGenerateArticleSummary(args) {
    ...
    async handleExportArticlesCSV(args) {
    ...
    async handleExportKnowledgeBase(args) {
